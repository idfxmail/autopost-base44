...
const {userPlatformDir}=require('../helpers/sessionStore');const {downloadImages}=require('../utils/downloadImages');const sel=require('../selectorMaps/yad2-sale.json');
async function withCookies(page,p){if(await fs.pathExists(p)){const c=await fs.readJson(p);await page.setCookie(...c);}}
module.exports=async function({user_id,property}){const dir=userPlatformDir(user_id,'yad2');const cookiesPath=path.join(dir,'cookies.json');
...
if(sel.total_floors)await page.type(sel.total_floors,String(property.total_floors||''),{delay:12}).catch(()=>{});
...
await page.goto(sel.my_ads_url,{waitUntil:'networkidle2'});const link=await page.evaluate(()=>{const a=document.querySelector('a[href*="item"]')||document.querySelector('a[href*="/item/"]');return a?a.href:null;});
return {link:link||'PUBLISHED_NO_LINK_FOUND',meta:{platform:'yad2-sale'}};} finally{await browser.close();}}
